...
export default function DisasterPreparednessPage() {
  ...
  const guides = useMemo(() => {
    const disasterGuides = allGuides.filter((g) => g.category === "Disaster Preparedness");
    if (!searchTerm) return disasterGuides;
    const term = searchTerm.toLowerCase();
    return disasterGuides.filter(
      (guide) =>
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground">Guides for earthquakes, floods, and more.</p>
            </div>
        </div>
