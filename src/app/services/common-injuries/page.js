...
export default function CommonInjuriesPage() {
  const [searchTerm, setSearchTerm] = useState("");

  const guides = useMemo(() => {
    const injuryGuides = allGuides.filter((g) => g.category === "Common Injuries");
    if (!searchTerm) return injuryGuides;
    const term = searchTerm.toLowerCase();
    return injuryGuides.filter(
      (guide) =>
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground">First aid for cuts, burns, sprains, and more.</p>
            </div>
        </div>
