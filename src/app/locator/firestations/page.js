...
export default function FireStationsPage() {
  ...
  const locations = useMemo(() => {
    const allFire = emergencyLocations.filter((l) => l.type === "fire");
    if (!searchTerm) return allFire;
    const term = searchTerm.toLowerCase();
    return allFire.filter(
      (location) =>
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground">Find nearby fire stations.</p>
            </div>
        </div>
        ...
        <section className="mt-8">
          {locations.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground mt-12">
              No fire stations found for your search term.
            </p>
          )}
        </section>
      </main>
