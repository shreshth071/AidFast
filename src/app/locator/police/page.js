...
export default function PolicePage() {
  ...
  const locations = useMemo(() => {
    const allPolice = emergencyLocations.filter((l) => l.type === "police");
    if (!searchTerm) return allPolice;
    const term = searchTerm.toLowerCase();
    return allPolice.filter(
      (location) =>
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground">Find nearby police stations.</p>
            </div>
        </div>
        ...
        <section className="mt-8">
          {locations.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground mt-12">
              No police stations found for your search term.
            </p>
          )}
        </section>
      </main>
