...
export default function DisasterPage() {
  ...
  const locations = useMemo(() => {
    const allDisaster = emergencyLocations.filter((l) => l.type === "disaster");
    if (!searchTerm) return allDisaster;
    const term = searchTerm.toLowerCase();
    return allDisaster.filter(
      (location) =>
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground">Find nearby disaster management centers.</p>
            </div>
        </div>
        ...
        <section className="mt-8">
          {locations.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground mt-12">
              No disaster centers found for your search term.
            </p>
          )}
        </section>
      </main>
