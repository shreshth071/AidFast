...
export default function PoisonControlPage() {
  ...
  const locations = useMemo(() => {
    const allPoison = emergencyLocations.filter((l) => l.type === "poison-control");
    if (!searchTerm) return allPoison;
    const term = searchTerm.toLowerCase();
    return allPoison.filter(
      (location) =>
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground">Find nearby poison control centers.</p>
            </div>
        </div>
        ...
        <section className="mt-8">
          {locations.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground mt-12">
              No poison control centers found for your search term.
            </p>
          )}
        </section>
      </main>
