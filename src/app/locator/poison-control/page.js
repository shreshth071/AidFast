...
export default function PoisonControlPage() {
  ...
  const poisonControlLocations = emergencyLocations.filter(l => l.type === 'poison-control');
  const [filteredLocations, setFilteredLocations] = useState(poisonControlLocations);
  ...
  const handleSearch = (event) => {
    ...
    if (term) {
      ...
      setFilteredLocations(poisonControlLocations);
    }
  };
    ...
    if (locations.length === 0) {
      return (
        <p className="text-center text-muted-foreground mt-4">
          No poison control centers found.
        </p>
      );
    }
    return (
    ...
    <div>
      <div className="mt-8 mb-6">
        <div className="relative">
          ...
          <Input
            type="search"
            placeholder="Search for a poison control center..."
